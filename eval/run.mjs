...
export function parseOptions(value) {
  ...
  if (options.browser_timeout_minutes * 60 <= options.task_timeout_seconds + 30)
    throw new Error(
      "Browser lifetime must exceed task timeout plus 30 seconds",
    );
  if (
    options.proxy_country_code !== null &&
    !/^[a-z]{2}$/.test(options.proxy_country_code)
  )
    throw new Error(
      "proxy_country_code must be a lowercase country code or null",
    );
  return options;
}
...
export async function main() {
  ...
  const cloudRequest = async (path, method, body) => {
    ...
    if (!response.ok)
      throw new Error(
        `Browser provider ${method} returned HTTP ${response.status}`,
      );
  ...
  try {
    ...
    if (
      Number(env.EVAL_TIMEOUT_MINUTES) * 60 <
      options.task_timeout_seconds + 90
    )
      throw new Error(
        "Platform timeout must exceed task timeout by at least 90 seconds",
      );
    if (
      !env.EVAL_MODEL_API_KEY ||
    ...
    const { BrowserUse, CDP, Page } = await import(
    ...
    const model = env.EVAL_MODEL.includes("/")
      ? env.EVAL_MODEL
      : `openai/${env.EVAL_MODEL}`;
    if (!model.startsWith("openai/"))
      throw new Error(
        "This eval adapter currently maps EVAL_MODEL_API_KEY to OpenAI only",
      );
            ...
            async onEvent(event) {
              ...
              if (
                ...
                modelSpan.setAttributes({
                  ...
                  [A.INPUT_COST]:
                    u.cost.input + u.cost.cacheRead + u.cost.cacheWrite,
                  [A.OUTPUT_COST]: u.cost.output,
                  [A.TOTAL_COST]: u.cost.total,
                });
                Laminar.withSpan(
                  modelSpan,
                  () => Laminar.setSpanOutput(clean(m)),
                  false,
                );
              ...
              if (event.type === "tool_execution_end") {
                ...
                if (span) {
                  Laminar.withSpan(
                    span,
                    () => Laminar.setSpanOutput(clean(event.result)),
                    false,
                  );
                  if (event.isError)
                    span.setStatus({
                  ...
                  try {
                    ...
                    if (target) {
                      ...
                      await writeFile(
                        join(
                          screenshots,
                          `${String(++screenshotIndex).padStart(3, "0")}.jpg`,
                        ),
    ...
    await writeFile(
      join(workspace, "sdk-result.json"),
